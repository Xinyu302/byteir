...
namespace mlir {
#define GEN_PASS_DEF_OPTIMIZEINTARITHMETIC
...
class OptimizeIntArithmeticPass
    : public impl::OptimizeIntArithmeticBase<OptimizeIntArithmeticPass> {
  using Base::Base;
  ...
  void runOnOperation() override {
    ...
    if (narrowToI32) {
    //   arith::populateArithIntNarrowingPatterns(patterns, solver, {32});
      patterns.add<NarrowSCFForIvToI32>(ctx, solver);
    }
    ...
    for (int i = 0;; ++i) {
      ...
      bool changed = false;
      if (failed(applyPatternsAndFoldGreedily(op, frozenPatterns, config, &changed))) {
        emitError(op->getLoc())
