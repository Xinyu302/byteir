...
namespace mlir {
#define GEN_PASS_DEF_GPUPIPELINING
...
static Operation *replaceOpWithPredicatedOp(RewriterBase &rewriter,
  ...
  if (!isa<nvgpu::DeviceAsyncCopyOp>(op)) {
    ...
    if (auto loadOp = dyn_cast<vector::LoadOp>(op)) {
      ...
      if (nvgpu::NVGPUDialect::hasSharedMemoryAddressSpace(loadBaseType))
        return op;
    }
...
static bool setPipeliningMarkers(scf::ForOp forOp, bool pipelineStoreStage) {
  ...
  for (Operation &op : forOp.getBody()->getOperations()) {
    ...
    if (!nvgpu::NVGPUDialect::hasSharedMemoryAddressSpace(stSrcType))
      continue;
    copyToWorkgroupMemory = true;
...
struct GPUPipeliningPass : impl::GPUPipeliningBase<GPUPipeliningPass> {
  using GPUPipeliningBase::GPUPipeliningBase;
