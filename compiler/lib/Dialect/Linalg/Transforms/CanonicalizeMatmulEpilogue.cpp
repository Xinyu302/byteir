...
modifyUseToGetValueIntoStoreSet(RewriterBase &rewriter,
  ...
  // get last block argument
  auto lastOperand = block->getArguments().back();
  if (!lastOperand.use_empty())
    return success();
  ...
  // inOperand and initOperand should be the same type
  if (inOperand->get().getType() != initOperand->get().getType())
    return success(); // do not modify
